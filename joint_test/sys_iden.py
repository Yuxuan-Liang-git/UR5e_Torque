...
# 1. 加载数据
data_path = "Data/Collection_data.csv"
...
if np.any(mask_s1):
    print("\n[Stage 1] 正在进行摩擦力对冲提取 (基于相同位置)...")
    dq_s1 = dq[mask_s1]
    tau_s1 = tau[mask_s1]
    q_s1 = q[mask_s1]
    
    # 使用较粗的精度进行聚类，以匹配我们设定的 10 个左右的工况点
    unique_vs = np.unique(np.round(np.abs(dq_s1), 1))
    unique_vs = unique_vs[unique_vs > 0.05] # 剔除接近停止的状态
    
    fric_data = [] # 存储为 (v, tau_f)
    
    # 对于每个目标速度，提取核心取值区 [-0.5, 0.5] rad
    # 利用公式 tau_f = (tau_pos(q) - tau_neg(q)) / 2
    for v_target in unique_vs:
        v_mask = (np.abs(np.abs(dq_s1) - v_target) < 0.05)
        # 核心 CV 探测区，排除掉加减速段的数据
        q_core_mask = (np.abs(q_s1) < 0.5)
        
        mask_pos = v_mask & q_core_mask & (dq_s1 > 0)
        mask_neg = v_mask & q_core_mask & (dq_s1 < 0)
        ...
        if np.any(mask_pos) and np.any(mask_neg):
            ...
            # 同时可以粗略估计 Bias = (tau_pos + tau_neg) / 2
            bias_est = (tau_pos_avg + tau_neg_avg) / 2.0
            
    fric_points = np.array(fric_data)
    if len(fric_points) > 0:
        # 使用这些纯摩擦数据点拟合 Stribeck
        # 拟合时，由于 Fs 难以在匀速段精确定义，将其作为一个自由参数 Fs_fit
        ...
        popt, _ = curve_fit(stribeck_fit_func, fric_points[:, 0], fric_points[:, 1], 
                             p0=[0.5, 0.1, 0.05, 1.0], bounds=([0,0,0.005,0], [2,5,0.5,2]))
        Fc_id, B_id, vs_id, Fs_id = popt
        print(f"辨识结果: Fs={Fs_id:.4f}, Fc={Fc_id:.4f}, B={B_id:.4f}, vs={vs_id:.4f}")
    else:
...
if np.any(mask_s2):
    ...
    J_id, bias_id = theta
    print(f"辨识结果: J={J_id:.6f}, Bias={bias_id:.6f}")
else:
...
plt.subplot(2, 1, 1)
if 'fric_points' in locals():
    # 绘制实测点（正反向对称展示）
    v_pts = fric_points[:, 0]
    f_pts = fric_points[:, 1]
    plt.scatter(v_pts, f_pts, color='blue', label='Extracted Points (+v)')
    plt.scatter(-v_pts, -f_pts, color='cyan', alpha=0.5, label='Mirrored Points (-v)')
    
    # 绘制完整拟合曲线
    v_range = np.linspace(-2.2, 2.2, 1000)
    # 完整 Stribeck 公式
...
plt.title("Stage 1: Complete Friction Identification (Symmetric Stribeck)")
...
plt.title(f"Stage 2: Overall Validation (RMSE: {np.sqrt(np.mean((tau-tau_pred)**2)):.4f} Nm)")
plt.xlabel("Time [s]"); plt.ylabel("Torque [Nm]"); plt.legend()

plt.tight_layout()
plt.savefig("Data/Semilinearized_Identification_Report.png")
plt.show()
