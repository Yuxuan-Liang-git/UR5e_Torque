    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            if not paused_state["value"]:
                ...
                # 夹爪随轨迹开闭: 完全张开时角度是0, 完全合并后是+0.064
                if gripper_actuator_id is not None:
                    # 使用正弦波逻辑实现 0 -> 0.064 的循环
                    # 这里可以让夹爪跟随 arm 的 8 字形周期同步开合
                    # sin 取值 [-1, 1] -> (sin+1)/2 取值 [0, 1] -> 范围 [0, 0.064]
                    gripper_target = 0.16 * (np.sin(w * t) + 1) + 0.16
                ...
                g_curr = data.qpos[gripper_joint_id] if gripper_joint_id is not None else 0.0
                plotter.update_data(data.time, pos_error, ori_error_vec, tau_clipped, data.qpos[dof_ids], q_des, g_curr, gripper_target)
