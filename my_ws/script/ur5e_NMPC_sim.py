...
def main():
    ...
    for k in range(N + 1): nmpc_controller.solver.set(k, "x", x0_warm)
    for k in range(N): nmpc_controller.solver.set(k, "u", np.zeros(nq))
    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            if not paused["value"]:
                ...
                # reference generator needs to supply MuJoCo frame target here because
                # compute_torque will do the internal conversion to Pinocchio
                # Update ref_queue to give us MuJoCo targets natively by bypassing its internal pinning
                # (Notice we have a design choice here, ref_queue stores pinocchio in memory rn)
                # For simplicity with NMPCController API we can generate them inline like torque_NMPC.py without using ref_queue
                
                ref_pos_batch = np.zeros((3, N + 1))
                ref_rot_batch = np.zeros((9, N + 1))
                for k in range(N + 1):
                    tk = t_traj + k * dt
                    pk = _get_pos(tk)
                    Rk = _get_ori(tk)
                ...
                # Error Calc (使用单独的FK计算以便做图表记录)
                ref_pos_pin = mj2pin_pos(ref_pos_batch[:, 0])
                ref_rot_pin = mj2pin_rot(ref_rot_batch[:, 0].reshape(3, 3, order='F'))
                
                ee_pin = f_fk_pos(q_cur).full().flatten()
                pos_err = ee_pin - ref_pos_pin
                ee_rot_pin = f_fk_rot(q_cur).full()
                R_err = ref_rot_pin.T @ ee_rot_pin
